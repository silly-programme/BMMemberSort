    ...
    [1, "MOAMETAL", [1,0,0,0,0,0,0,0], "bmppl/Moa.jpg"]
      ...
      for (i=0; i<ary_TempData.length; i++) {
         ...
         if ((int_ResultImg != 0) && (i < int_ResultRank)) {
            var new_img = cE('img');
            var obj_TempData = ary_TempData[i];
         ...
         if ((int_ResultImg == 2) || (!bln_imgFlag)) {
            new_cell.appendChild(cT(ary_TempData[i][1]));
			csort4[i] = ary_TempData[i][1]; // v2a
			csort6[i] = ary_TempData[i][1]; // v2a
         }

         if (i < ary_TempData.length - 1) {
            if (bln_ResultMode == 0) {
               if (ary_EqualData[i] == ary_SortData[0][i + 1]) {
                  int_Result++;
               }
            } else {
               if (ary_EqualData[i] == ary_SortData[0][i + 1]) {
